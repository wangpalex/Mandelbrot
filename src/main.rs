...
fn main() {
    ...
    // _concurrent_render(&mut pixels, bounds, upper_left, lower_right);
    _rayon_render(&mut pixels, bounds, upper_left, lower_right);
...
fn _rayon_render(
    ...
    // break pixels into rows attached with row number
    let bands: Vec<(usize, &mut [u8])> = pixels.chunks_mut(bounds.0).enumerate().collect();

    // let rayon manage parallel workloads
